...
@section('title', 'Add Loan Device - ' . config('app.name'))
...
<div class="container" style="max-width: 560px;">
    ...
    <div class="card">
        <form method="POST" action="{{ route('loan-devices.store') }}">
            @csrf
            <div class="form-group">
                ...
                <div class="device-select-wrap">
                    <input type="text" id="device_search" class="device-select-input" autocomplete="off" placeholder="Type to search by product code, make or model..."
                        value="">
                    <input type="hidden" name="device_id" id="device_id" value="{{ old('device_id') }}" required>
            ...
            <div class="form-group">
                ...
                <input type="text" id="imei" name="imei" value="{{ old('imei') }}" maxlength="50" placeholder="e.g. 15 digits">
                @error('imei')<div class="error">{{ $message }}</div>@enderror
            </div>
            <div class="form-actions">
                <a href="{{ route('loan-devices.index') }}" class="btn btn-secondary">Cancel</a>
                <button type="submit" class="btn btn-primary">Add loan device</button>
            </div>
        </form>
    </div>
</div>
...
(function() {
    ...
    var selectedId = {{ json_encode(old('device_id')) }};
